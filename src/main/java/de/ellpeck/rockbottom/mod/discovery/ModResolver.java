...
import de.ellpeck.rockbottom.mod.loader.metadata.ModDependency;
import de.ellpeck.rockbottom.mod.metadata.LoaderModMetadata;
import de.ellpeck.rockbottom.mod.metadata.ModMetadataParser;
...
//			logger.warn("Warnings were found! " + errSoftStr);
	...
	static class UrlProcessAction extends RecursiveAction {
		...
		UrlProcessAction(Map<String, ModCandidateSet> candidatesById, URL url, int depth) {
...
//									loader.getLogger().debug("Found nested JAR: " + modPath);
					...
					if (!jarInJars.isEmpty()) {
						invokeAll(
							jarInJars.stream()
								.map((p) -> {
									try {
										return new UrlProcessAction(candidatesById, UrlUtil.asUrl(p.normalize()), depth + 1);
									} catch (UrlConversionException e) {
	...
	public Map<String, ModCandidate> resolve(GameProvider gameProvider) throws ModResolutionException {
		...
		for (ModCandidateFinder f : candidateFinders) {
			f.findCandidates((u) -> {
				UrlProcessAction action = new UrlProcessAction(candidatesById, u, 0);
		...
		// add builtin mods
		for (GameProvider.BuiltinMod mod : gameProvider.getBuiltinMods()) {
