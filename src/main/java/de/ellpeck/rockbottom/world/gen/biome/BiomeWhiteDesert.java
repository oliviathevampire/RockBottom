...
public class BiomeWhiteDesert extends BiomeBasic {

    public BiomeWhiteDesert(ResourceName name, int weight, BiomeLevel... levels) {
        ...
        if (layer == TileLayer.MAIN || layer == TileLayer.BACKGROUND) {
            ...
            if (chunk.getY() + y <= surfaceHeight) {
                if (chunk.getY() + y >= surfaceHeight - Util.ceil(noise.make2dNoise(worldX / 5D, 0D) * 3D)) {
                    return GameContent.TILE_WHITE_SAND.getDefState();
                } else {
                    return GameContent.TILE_WHITE_SANDSTONE.getDefState();
                }
            }
        }
