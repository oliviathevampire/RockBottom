...
public class ItemBronzeCanister extends ItemBasic {

    public ItemBronzeCanister() {
        super(ResourceName.intern("bronze_canister"));
    }
    ...
    public void describeItem(IAssetManager manager, ItemInstance instance, List<String> desc, boolean isAdvanced, boolean isRealItem) {
        ...
        desc.add(manager.localize(ResourceName.intern("info.bronze_canister")));
    }
    ...
    public boolean onInteractWith(IWorld world, int x, int y, TileLayer layer, double mouseX, double mouseY, AbstractEntityPlayer player, ItemInstance instance) {
		...
		if (!recipe.isKnown(player)) {
            ...
            player.openGui(new GuiMessageBox(null, 0.25F, 200, 18, new ChatComponentTranslation(ResourceName.intern("info.bronze_canister.note"))));
        } else {
            player.openGui(new GuiMessageBox(null, 0.25F, 200, 18, new ChatComponentTranslation(ResourceName.intern("info.bronze_canister.empty"))));
        }
